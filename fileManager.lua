local FileManager = {}
local function getColorIndex(palette, tile)
    local index = 1
    for _, line in ipairs(palette.colorTiles) do
        for _, color in ipairs(line) do
            if color.color[1] == tile.color[1] and color.color[2] == tile.color[2] and color.color[3] == tile.color[3] then
                return index
            end
            index = index + 1
        end
...
function FileManager.saveSprite(name, grid, palette)
    ...
    for y = 1, #grid do
        fileData = fileData .. "    { "
        for x = 1, #grid[y] do
            local index = getColorIndex(palette, grid[y][x])
            if index then
                fileData = fileData .. string.format("%i,", index)
            end
        end
        fileData = fileData .. "},\n"
    end
